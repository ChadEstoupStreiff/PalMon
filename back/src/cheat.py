...
async def cheat_eggs(user: str, quantity: int):
    requests.post(f"http://palmon_service_eggs:80/egg?owner={user}&quantity={quantity}").json()

@router.post("/cheat/dollars")
async def cheat_dollars(user: str, amount: int):
    requests.post(f"http://palmon_service_shop:80/money?user={user}&amount={amount}").json()
