...
from db import DB
from objects import Palmon, BagSlot, StorageSlot
...
async def switch_bag_storage(user: str, palmon_id: int):
    ...
    bag = commit.query(BagSlot).filter(BagSlot.owner == user).filter(BagSlot.palmon_id == palmon_id).first()
    if bag:
        commit.delete(bag)
    ...
    storage = commit.query(StorageSlot).filter(StorageSlot.owner == user).filter(StorageSlot.palmon_id == palmon_id).first()
    if storage:
...
async def endpoint_bag_delete(user: str, palmon_id: int):
    commit = DB().get()
    storage = commit.query(BagSlot).filter(BagSlot.owner == user).filter(BagSlot.palmon_id == palmon_id).first()
    commit.delete(storage)
    commit.commit()
    return True
...
def get_storage_of(user: str) -> List[Palmon]:
    return DB().get().query(Palmon).join(StorageSlot).filter(StorageSlot.owner == user).all()
